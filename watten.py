    ...
    if (len(potentiell_angesagte) == 0):
        for farbe in range(0, 4):
            for schlag in range(0, 8):
                ...
                if (np.array_equal(stich(a, b, c, d, versuche),
                                   stechende_karte)):  # es werden einfach alle karten getestet, wenn die stechende Karte mit der tatsächlich stechenden übereinstimmt, wird sie zum Array der potentiellen Angesagten hinzugefügt.
                    potentiell_angesagte.append(versuche)
        if (np.array_equal(stich(a, b, c, d, (0, 8)), stechende_karte)):
            potentiell_angesagte.append(
                (0, 8))  # weli wird nochmal extra überprüft weil er nicht in die Schleife passt.
    else:
        potentiell_angesagte[:] = [x for x in potentiell_angesagte if np.array_equal(stich(a, b, c, d, x),
                                                                                     stechende_karte)]  # list comprehension for the win
...
def eingabe(string):
    if string == "Schell" or string == "s":
        return 0
    elif string == "Herz" or string == "h":
        return 1
    elif string == "Eichel" or string == "e":
        return 2
    elif string == "Laab" or string == "l":
        return 3
    elif string == "Sieben" or string == "7":
        return 0
    elif string == "Acht" or string == "8":
        return 1
    elif string == "Neun" or string == "9":
        return 2
    elif string == "Zehn" or string == "10":
        return 3
    elif string == "Unter" or string == "u":
        return 4
    elif string == "Ober" or string == "o":
        return 5
    elif string == "König" or string == "k":
        return 6
    elif string == "Ass" or string == "a":
        return 7
    elif string == "Weli" or string == "w":
        return 8
    else:
